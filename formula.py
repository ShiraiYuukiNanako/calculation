...
def AOU(T,S,O2):
    return mat.exp(177.7888 + 255.5907 / (T / 100) + 146.4813 * mat.log(T / 100, mat.e) - 22.204 * (T / S * (0.037362 +
...
def radioactive_decay(N0,Lambda,t):
    return N0 * mat.exp(-Lambda * t)

def radioactive_growth(Np,Lambda,t):
    return Np*(mat.exp(Lambda * t) - 1)

def mean_life(Lambda):
    return 1/Lambda

def half_life(Lambda):
    return mat.log(2, mat.e) / Lambda
...
def free_metal(Ctotal,logbeta_list,C_list,EDTA,Rinorganic):
    ...
    Y4 = EDTA/count
    result = Ctotal/(mat.pow(10, Rinorganic) + mat.pow(10, logbeta_list[4]) * Y4)
